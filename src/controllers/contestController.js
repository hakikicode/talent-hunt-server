...
exports.voteForParticipant = async (req, res) => {
  ...
  try {
    ...
    // Check if the participant is part of the contest
    const participant = contest.participants.find(
      (p) => p.toString() === participantId
    );
    if (!participant) {
      return res.status(404).send({ message: "Participant not found in the contest" });
    }

    // Check if user has already voted (optional logic depending on rules)
    // For example, if users can only vote once per contest:
    const userVote = contest.votes.find(
    ...
    // Add the vote to the contest
    contest.votes.push({ userId, participantId, votes });
    ...
    console.error(error);
    res.status(500).send({ message: error.message || "An error occurred while voting" });
  }
};
...
exports.registerParticipant = async (req, res) => {
  ...
  try {
    ...
    res.status(200).send({ message: "Participant registered successfully" });
  } catch (error) {
    res
      .status(500)
      .send({ message: error?.message || "An error occurred while registering participant" });
  }
};
